...
describe("authenticateServiceRequest Middleware", () => {
    ...
    test("should return 403 if useRole is true and token does not have SYSTEM_ADMIN role", async () => {
        ...
        await authenticateServiceRequest({AdminRole: true})(mockRequest, mockResponse, nextFunction);
    ...
    test("should add 'AUTH_SERVICE' to allowedRoles when AuthService option is true", async () => {
        const mockToken = "valid.token.here";
        ...
        // Mock header dan token
        mockRequest.headers["authorization"] = `Bearer ${mockToken}`;
        jwt.verify.mockReturnValue(decodedPayload); // Mock hasil decoding token

        // Jalankan middleware dengan opsi AuthService: true
        await authenticateServiceRequest({AuthService: true})(mockRequest, mockResponse, nextFunction);

        // Pastikan next() dipanggil karena role AUTH_SERVICE ada di token
