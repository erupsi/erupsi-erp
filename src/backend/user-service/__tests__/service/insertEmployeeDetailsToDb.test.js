...
describe('insertEmployeeDetailsToDb', () => {
  ...
  it('should insert employee and role details successfully and commit the transaction', async () => {
    // Arrange
    const employeeData = { employeeId: 'emp-001', fullName: 'Rina Kartika', email: 'rina.k@example.com', department: 'Marketing', position: 'Digital Strategist', roleName: 'editor' };
  ...
  it('should rollback the transaction if the role name does not exist', async () => {
    ...
    const employeeData = { employeeId: 'emp-002', fullName: 'Budi Santoso', email: 'budi.s@test.com', department: 'IT', position: 'Support', roleName: 'non-existent-role' };
    client.query.mockImplementation(query => {
  ...
  it('should rollback the transaction if inserting into employees table fails', async () => {
    ...
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
    const employeeData = { employeeId: 'emp-003', fullName: 'Citra Lestari', email: 'citra.l@test.com', department: 'HR', position: 'Recruiter', roleName: 'hr-staff' };
