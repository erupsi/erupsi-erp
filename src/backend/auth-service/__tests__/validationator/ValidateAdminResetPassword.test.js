const validateAdminResetPassword = require('../../src/validationator/validateAdminResetPassword');
...
const runValidation = async (req) => {
  ...
  // This is where validationResult is checked and `next()` is called if no errors
...
describe('validateAdminResetPassword', () => {
  ...
  test('should fail if body contains an invalid key', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      error: ['Properti yang tidak diizinkan terdeteksi: invalidKey. Hanya username, password yang diizinkan.'],
    });
  });
  ...
  test('should fail if password does not contain an uppercase letter', async () => {
    const req = mockRequest({
      username: 'admin',
      password: 'password123!',
    });
  ...
  test('should fail if password does not contain a lowercase letter', async () => {
    const req = mockRequest({
      username: 'admin',
      password: 'PASSWORD123!',
    });
