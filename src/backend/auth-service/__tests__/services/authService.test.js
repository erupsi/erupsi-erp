...
describe('Auth Service Functions', () => {
  ...
  describe('registerUser', () => {
    ...
    test('should return success: true on successful registration', async () => {
      // Skenario sukses: checkEmployeeByUsername tidak menemukan user, lalu insert berhasil
      pool.query
        .mockResolvedValueOnce({ rows: [] }) // Panggilan pertama dari checkEmployeeByUsername
        .mockResolvedValueOnce({}); // Panggilan kedua dari INSERT
      bcryptSalting.mockResolvedValue('hashed_password');
      
      const result = await registerUser('mock-uuid', 'testuser', 'password123', new Date());

      expect(pool.query).toHaveBeenCalledTimes(2);
      expect(pool.query).toHaveBeenCalledWith('SELECT * FROM auth_employee WHERE username=$1', ['testuser']);
      expect(bcryptSalting).toHaveBeenCalledWith('password123');
      expect(result).toEqual({ success: true });
    });

    test('should return success: false if username already exists', async () => {
      ...
      expect(result).toEqual({ success: false, message: 'User Already Defined' });
    });

    test('should return success: false on a database error', async () => {
      // Skenario: query pertama (checkEmployeeByUsername) berhasil, tapi insert gagal
      pool.query
  ...
  describe('checkEmployeeByUsername', () => {
    test('should return employee data if user is found', async () => {
      const mockResult = { rows: [{ employeeId: 'id-123', username: 'testuser' }] };
  ...
  describe('AddEmployeeRequestToUrm', () => {
    ...
    test('should return a success response from URM', async () => {
      requestBuilder.mockResolvedValue({ success: true, data: { employeeId: 'id-123' } });
      ...
      expect(result).toEqual({ success: true, data: { employeeId: 'id-123' } });
    });
  ...
  describe('getEmployeeUsername', () => {
    test('should return the username for a given employeeId', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith('SELECT username FROM auth_employee WHERE employeeId=$1', ['id-123']);
  ...
  describe('changeEmployeePassword', () => {
    test('should return success: true on a successful password change', async () => {
      pool.query.mockResolvedValue({});

      const result = await changeEmployeePassword('testuser', 'new_hashed_password', new Date());
