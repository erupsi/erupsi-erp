const { csrfProtection, csrfHandler, handlerErrorCsrf } = require('../../src/middlewares/csrfProtect'); // Sesuaikan path jika perlu
...
describe('CSRF Middleware and Handlers', () => {
  ...
  describe('handlerErrorCsrf', () => {
    test('should return 403 Forbidden with a custom error message for EBADCSRFTOKEN', async () => {
      const req = mockRequest();
      const res = mockResponse();
      const error = { code: 'EBADCSRFTOKEN' }; // Simulasikan error dari csurf
      ...
      expect(res.json).toHaveBeenCalledWith({ error: 'Token CSRF tidak valid. Permintaan diblokir.' });
    ...
    test('should call next() for non-CSRF errors', async () => {
      const req = mockRequest();
      const res = mockResponse();
      const error = new Error('Some other error'); // Simulasikan error non-csurf
