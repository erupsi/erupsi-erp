...
describe('loginEmployee Controller', () => {
  ...
  const employeeData = {
    ...
    password_expiry: new Date(new Date().getTime() + 7 * 24 * 60 * 60 * 1000), // 7 days from now
  };
  const urmData = { data: { roles: ['USER'] } };
  const tokens = { refreshToken: 'mock-refresh-token', accessToken: 'mock-access-token' };
  ...
  // --- Skenario Berhasil ---
  test('should return 200 and set cookies for a successful login', async () => {
    ...
    expect(passwordUtils.comparator).toHaveBeenCalledWith('validpassword', 'hashedpassword');
    expect(authService.getEmployeeDataFromUrm).toHaveBeenCalledWith('emp-123');
    expect(refreshTokenService.tokenBuilderAssigner).toHaveBeenCalledWith(
      mockResponse, 'emp-123', 'testuser', ['USER'], { replace_token: true }
    );
  ...
  test('should return 200 with a password expiry warning', async () => {
    const expiringEmployeeData = {
      ...employeeData,
      password_expiry: new Date(new Date().getTime() + 2 * 24 * 60 * 60), // 2 days from now
    };
    authService.checkEmployeeByUsername.mockResolvedValue(expiringEmployeeData);
    passwordUtils.comparator.mockResolvedValue(true);
    authService.getEmployeeDataFromUrm.mockResolvedValue(urmData);
    refreshTokenService.tokenBuilderAssigner.mockResolvedValue(tokens);
    ...
    expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({
      message: 'Login successful',
      passwordExpiryWarning: 'Your password will expire in 0 days. Please change it soon.',
    }));
  });
  ...
  test('should return 200 with a password expired message', async () => {
    const expiredEmployeeData = {
      ...employeeData,
      password_expiry: new Date(new Date().getTime() - 1 * 24 * 60 * 60 * 1000), // 1 day ago
    };
    authService.checkEmployeeByUsername.mockResolvedValue(expiredEmployeeData);
    passwordUtils.comparator.mockResolvedValue(true);
    authService.getEmployeeDataFromUrm.mockResolvedValue(urmData);
    refreshTokenService.tokenBuilderAssigner.mockResolvedValue(tokens);
  ...
  test('should return 401 if username is not found', async () => {
    ...
    expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Invalid username or password' });
  ...
  test('should return 500 for an internal server error', async () => {
    ...
    expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Internal server error At Login Employee' });
  });
});