...
describe("Controller: refreshAccessToken", () => {
    ...
    test("should return 401 if refresh token is not found in cookies",
        async () => {
            mockRequest.cookies = {};
    ...
    test("should return 403 if refresh token is not found in the database",
        async () => {
            mockRequest.cookies.refreshToken = "nonexistent-token";
            searchRefreshToken.mockResolvedValue([]);

            await refreshAccessToken(mockRequest, mockResponse, mockNext);

            expect(searchRefreshToken)
                .toHaveBeenCalledWith("nonexistent-token");
            expect(mockResponse.status).toHaveBeenCalledWith(403);
            expect(mockResponse.json).toHaveBeenCalledWith(
                {error: "Invalid token. Please login again."});
        });

    test(`should return 403 and delete user 
        tokens if the token is marked as invalid`, async () => {
    ...
    test("should return 403 and delete the token if it has expired",
        async () => {
            const pastDate = new Date();
            pastDate
                .setHours(pastDate.getHours() - 1); // Set waktu 1 jam yang lalu
    ...
    test("should successfully refresh the token and return a new access token",
        async () => {
            ...
            mockRequest.cookies.refreshToken = "valid-token";
            searchRefreshToken.mockResolvedValue([mockTokenData]);
            getEmployeeDataFromUrm.mockResolvedValue(
            ...
            invalidateAndInsertToken.mockResolvedValue(undefined);
            ...
            expect(tokenBuilderAssigner)
                .toHaveBeenCalledWith(mockResponse, "emp-123",
                    "testuser", ["admin", "user"]);
